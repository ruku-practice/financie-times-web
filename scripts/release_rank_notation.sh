#!/bin/bash
# =============================================================================
# v3.2.4（日付別ランキングの表記整理＋価格ランキング2種）を本番（GitHub Pages）へ出す「押せば出る」スクリプト（release_merged.sh の写し・2026-09-13）
#
#   既定＝ドライラン：何をするかを全部印字して、何も変えない（git fetch だけは行う＝origin の最新を知るため）
#   実行＝  bash scripts/release_rank_notation.sh --execute
...
#     P4 v3.2.4 の中身が入っている＝index.html に並べ替え釦 data-sort が4つ・js/advanced.js に DAILY_SORTS がある
#
#   やること
#     ①worktree（feature/rank-notation-3.2.4）に origin/main を取り込む（衝突するなら止まる・stash 禁止）
...
#     ④GitHub Pages の反映を curl で確認（v=VERSION・JS／CSS の SHA256 一致・data/overview の JSON 200・history.json は状態表示）
#     ⑤progress.md / VERIFIED.md に貼る1行の雛形を印字
#
#   bash 3.2（macOS 標準）で動く。作成 2026-09-13 12:00 JST（合体の担当）→ v3.2.4 用に写し 13:45 JST（表記整理の担当）。
# =============================================================================
set -euo pipefail

VERSION="3.2.4"
PRODUCT_DIR="/Users/kkr/ruku_data/00_products/apps/FiNANCiE-times-web"
WORKTREE="$PRODUCT_DIR/.claude/worktrees/rank-notation"
BRANCH="feature/rank-notation-3.2.4"
PROD_URL="https://ruku-practice.github.io/financie-times-web"
GIT_ID=(-c user.name=kkr -c user.email=kkr@kkr-MacM5Max.local)
CHECK_FILES="js/advanced.js css/layout.css js/overview.js js/analysis.js"
...
say "=== v${VERSION} 本番リリース ｜ モード＝${MODE} ｜ $(now) JST ==="
...
SORT_BTNS="$(grep -c 'class="sort-tab-btn[^"]*" data-sort="' index.html || true)"
if [ "$SORT_BTNS" != 4 ] || ! grep -q "const DAILY_SORTS" js/advanced.js; then
  MISSING="${MISSING}
  P4 v3.2.4 の並べ替え4種が入っていない（index.html の data-sort 釦 ${SORT_BTNS}個・DAILY_SORTS の有無）"
fi
if [ -n "$MISSING" ]; then
...
if [ "$BEHIND" = 0 ]; then
  ...
  if git merge-tree --write-tree --quiet "$LOCAL_SHA" "$REMOTE_SHA" >/dev/null 2>&1; then
    ...
    run git "${GIT_ID[@]}" merge --no-edit --no-ff -m "merge: origin/main を取り込み（v${VERSION} リリース前）" origin/main
  else
...
if [ "$REBUILD" = 1 ]; then
  ...
  if [ "$EXECUTE" = 1 ]; then
    ...
    if [ -n "$(git status --porcelain -- data/overview)" ]; then
      ...
      git "${GIT_ID[@]}" commit --quiet -m "chore: 総覧データ（data/overview・schema 2）を再生成（$(now) JST・v${VERSION} リリース前）"
...
if [ "$EXECUTE" = 1 ]; then
  ...
  PUB_SORT="$(curl -s "${PROD_URL}/?r=${RANDOM}" | grep -c 'data-sort="price_rate"\|data-sort="price_diff"' || true)"
  say "  ✅ 本番の並べ替え釦（価格の上昇率順・上昇額順）: ${PUB_SORT}個（2 が正）"
  [ "$PUB_SORT" = 2 ] || stop "本番に価格の並べ替え釦が2つ見えません（${PUB_SORT}個）"
else
...
say "- $(now) JST｜稼働中｜ **日付別ランキングの表記整理＋価格ランキング v${VERSION} を本番（GitHub Pages）へ反映**（コミット $(git rev-parse --short "$PUSHED_SHA")・scripts/release_rank_notation.sh） ／ 検証：curl で ${PROD_URL}/ に v=${VERSION}・JS／CSS の SHA256 一致・data/overview の JSON 200 ／ 次：翌朝の定期実行で data/overview が更新されるかを確認"
say "--- VERIFIED.md（表に1行）---"
say "| ✅ 日付別ランキングの表記整理＋価格ランキング v${VERSION} を本番（GitHub Pages）へ反映 | $(TZ=Asia/Tokyo date +%F) | コミット \`$(git rev-parse --short "$PUSHED_SHA")\`・\`scripts/release_rank_notation.sh --execute\`。curl：\`${PROD_URL}/\` に \`v=${VERSION}\`・${CHECK_FILES} の SHA256 一致・data/overview の JSON 200 |"
say ""
if [ "$EXECUTE" = 1 ]; then
  say "=== 完了 $(now) JST ==="
else
  say "=== ドライラン終了（何も変えていません）。実行は: bash scripts/release_rank_notation.sh --execute ==="
fi
